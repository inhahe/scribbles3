...
//extern "C" {static uint8_t* pixels = nullptr; }
//extern "C" {int pixels[w * h * 4]; }
static uint8_t pixels[w * h * 4];
extern "C" {uint8_t* EMSCRIPTEN_KEEPALIVE getpixels() { return &pixels[0]; }}     
//int* EMSCRIPTEN_KEEPALIVE getpixels() { return (int*)pixels; }
//EMSCRIPTEN_BINDINGS(graphics) static uint8_t pixels[w * h * 4];
...
void drawscreen(vector<point> disppoints)
{
  ...
  memset(screen, 0, w * h * sizeof(bool)); //debug
  ...
  int bgint_image = bg.r + (bg.g << 8) + (bg.b << 16) + 0xff000000;
  int fgint_image = fg.r + (fg.g << 8) + (fg.b << 16) + 0xff000000;
  ...
  for (int y = 0; y < h; y++)
  {
    ...
    for (int x = 0; x < w; x++)
    {
      ...
      if (dot)
      {
        *(uint32_t*)sp_pixels = fgint_image;
      }
      else
      {
        *(uint32_t*)sp_pixels = bgint_image;
      }
      sp_pixels += 4;
      sp_image += 4;
    }
  }
  for (int i = 0; i < w * h * 4; i++) //debug
  {
    if (pixels[i] != 0)
    {
      //prnjs("here 1");
  ...
  //update window, or actually it should be updated automatically using the shared 'pixels' after mainloop fininshes making a pass.
}
...
extern "C"
{
  EMSCRIPTEN_KEEPALIVE void mainloop()
  {
...
extern "C"
{
  EMSCRIPTEN_KEEPALIVE void update(int curves, int bgr, int bgg, int bgb, int fgr, int fgg, int fgb, bool rothue, int huespd, int huesat, int hueval, bool cont, int spacepoints, int timepoints)
  {
    bool doinit = false;
    if (timecurves != timecurvepoints || contiguous != cont) doinit = true;
    spacecurves = curves;
    bg.r = bgr;
    bg.g = bgg;
    bg.b = bgb;
    ...
    val = hueval;
    contiguous = cont;
    spacecurvepoints = spacepoints;
    timecurvepoints = timepoints;
