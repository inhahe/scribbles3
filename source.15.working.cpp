...
int main(int argc, char* argv[])
{
  ...
  if (argc == 1)
  {
    ...
    std::cout << "  --bgcolor <background color, e.g. #ffffff>" << endl;
    std::cout << "  --fgcolor <foreground color, e.g. #0000ff>" << endl;
    std::cout << "  --noloop - doesn't loop; continously makes new shapes. --file and --timepoints are ignored if this is specified." << endl;
    std::cout << endl;
  ...
  if (noloop)
  {
    ...
    {
      mps[i] = metapoints(w, h, pointsperspacecurve);
    }
    for (;;)
    {
      ...
      for (int i = 0; i < spacecurves; i++)
      {
        dispanchors.push_back(mps[i].getpoint());
      }
      SDL_RenderPresent(renderer);
