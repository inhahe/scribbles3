...
public:
  ...
  metapoints(const int w, const int h, int pointspercurve, int timecurves, int timecurvepoints)
  {
    auto timeanchors = randanchors(w, h, timecurves);
    this->timepoints = createpercloop(timeanchors, timecurvepoints);
    this->pointindex = 0;
    this->noloop = false;
  }
  ...
  point getpoint()
  {
    ...
    {
      this->pointsindex = (this->pointsindex + 1) % this->timepoints.size();
      return this->timepoints[this->pointsindex];
    }
  }
  ~metapoints()
  {
...
public:
  ...
  circle(int w, int h, int pointspercurve, int timecurves, default_random_engine generator, float speed_dist_mean, float speed_dist_sd, float size_dist_mean, float size_dist_sd)
  {
    int bs = testbeziersize(timecurvepoints);
    //default_random_engine generator(rand()); 
    double cycles_dist_mean = pointspercurve * timecurves / size_dist_mean * speed_dist_mean;
    normal_distribution<double> cycles_distribution(cycles_dist_mean, cycles_dist_mean / 2.0);
    normal_distribution<double> size_distribution(size_dist_mean, size_dist_sd);
    this->size = size_distribution(generator);
    this->speed = int(cycles_distribution(generator)) * float(this->size) / (bs * timecurves);
    // while (this->speed <= 0) this->speed = speed_distribution(generator);// why tf doesn't this work?
     //while (this->size <= 0) this->size = size_distribution(generator);
    this->phase = 0;
    this->mp = metapoints(w, h, pointspercurve, timecurves, timecurvepoints);
  }
...
int main(int argc, char* argv[])
{
  ...
  else
  {
    ...
    {
      generator = default_random_engine(rand());
      circles[i] = circle(w, h, timecurvepoints, timecurves, generator, speed_dist_mean, speed_dist_sd, size_dist_mean, size_dist_sd);
    }
    int i = 0;
    int bs = testbeziersize(timecurvepoints);
    if (dowrite) image = new uint8_t[w * h * 4];
    for(;;)
    {
      for (int i = 0; i < num_circles; i++)
      {
