...
public:
  ...
  metapoints(const int w, const int h, int pointspercurve)
  {
    ...
    auto curvepoints = beziercurve_quadratic(this->c2.p1, this->c2.p2, this->c2.p3, pointspercurve);
  ...
  point getpoint()
  {
    ...
    if (this->pointindex >= this->cpsize)
    {
      ...
      cout << "c1.p1: " << c1.p1.x << " " << c1.p1.y << " c1.p2: " << c1.p2.x << " " << c1.p2.y << " c1.p3: " << c1.p3.x << " " << c1.p3.y << endl;
      ...
      cout << "c2.p1: " << c2.p1.x << " " << c2.p1.y << " c2.p2: " << c2.p2.x << " " << c2.p2.y << " c2.p3: " << c2.p3.x << " " << c2.p3.y << endl;

      this->pointindex = 0;
      auto curvepoints = beziercurve_quadratic(this->c2.p1, this->c2.p2, this->c2.p3, this->pointspercurve);
      cout << "curvepoints.front(): " << curvepoints.front().x << " " << curvepoints.front().y << endl;
      point lp = this->lastpoint;
      cout << "lp: " << lp.x << " " << lp.y << endl;;
      cout << "curvepoints2.size(): " << curvepoints2.size();
      //cout << "curvepoints2.back(): " << curvepoints2.back().x << " " << curvepoints2.back().y << endl;
      for (auto &p : curvepoints)
      {
        auto linepoints = plotline(lp, p);
        this->curvepoints2.insert(this->curvepoints2.end(), linepoints.begin(), linepoints.end());
        //curvepoints2.push_back(p);
        vector<point>().swap(linepoints); //free memory
        lp = p;
      }
      this->lastpoint = curvepoints.back();
      vector<point>().swap(curvepoints);
      cout << "curvepoints2.front(): " << curvepoints2.front().x << " " << curvepoints2.front().y << endl;
      this->cpsize = this->curvepoints2.size();
      return this->curvepoints2[this->pointindex];
      //cout << "curvepoints2.size(): " << this->curvepoints2.size() << endl;
    }
    else
    {
      return this->curvepoints2[this->pointindex++];
...
void drawscreen(SDL_Renderer* renderer, int w, int h, point curvepoints[])
{
  ...
  {
    auto curvepoints2 = beziercurve_quadratic(c.p1, c.p2, c.p3, pointspercurve);
...
int main(int argc, char* argv[])
{
  ...
  for(;;)
  {
    for (int i = 0; i < 1; i++)
    {
      curvepoints[i] = mps[i].getpoint();
      SDL_RenderDrawPoint(renderer, curvepoints[i].x, curvepoints[i].y);
    }
    SDL_RenderPresent(renderer);
