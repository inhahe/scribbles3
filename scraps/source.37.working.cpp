...
void drawscreen(SDL_Renderer* renderer, int w, int h, vector<point> disppoints,
  bool* screen, uint8_t* image, GifWriter& writer, bool noscreen, bool dowrite, rgb bg, rgb fg, SDL_PixelFormat* pixel_format, SDL_Texture* texture, SDL_Window* window)
{
  ...
  if (noloop)
  {
    ...
    for (;;)
    {
      ...
      drawscreen(renderer, w, h, createdisploop(createpercloop(dispanchors, spacecurvepoints)),
        screen, image, writer, noscreen, dowrite, bg, fg, pixel_format, texture, window);
      if (rotatehue)
      {
        hue += huespeed;
