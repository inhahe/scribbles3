...
extern "C" {uint8_t* EMSCRIPTEN_KEEPALIVE getpixels() { return pixels; }}     
...
extern "C"
{
  EMSCRIPTEN_KEEPALIVE void update(int size, int curves, int bgr, int bgg, int bgb, int fgr, int fgg, int fgb, bool rothue, int huespd, int huesat, int hueval, bool cont, int spacepoints, int timepoints)
  {
    bool doinit = false;
    if (timecurves != curves || contiguous != cont || w!=size) doinit = true;
    w = size;
    h = size;
    spacecurves = curves;
    bg.r = bgr;
