...
void parsecommandline(int argc, char* argv[])
{
  try
  {
    ...
    if (vm.count("spacepoints"))
    {
      spacecurves = vm["spacepoints"].as<int>();
    }
    if (vm.count("timepoints"))
    {
      timecurves = vm["timepoints"].as<int>();
    }
    if (vm.count("w"))
    {
      w = vm["w"].as<int>();
    ...
    if (vm.count("bgcolor"))
    {
      bg = hex2rgb(vm["bgcolor"].as<string>());
    }
    if (vm.count("fgcolor"))
    {
      fg = hex2rgb(vm["fgcolor"].as<string>());
    }
    if (vm.count("loop"))
    {
      noloop = false;
    }
    if (vm.count("incontiguous"))
    {
      contiguous = false;
    }
    if (vm.count("pointsperspacecurve"))
    {
      pointspertimecurve = vm["pointsperspacecurve"].as<int>();
    }
    if (vm.count("pointsperstimecurve"))
    {
      pointspertimecurve = vm["pointspertimecurve"].as<int>();
    }
  }
