...
public:
  ...
  metapoints(const int w, const int h, int pointspercurve, bool contiguous)
  {
    ...
    this->curvepoints = beziercurve_quadratic(this->c2.p1, this->c2.p2, this->c2.p3, pointspercurve);
...
int main(int argc, char* argv[])
{
  ...
  if (argc == 1)
  {
    ...
    std::cout << "  --seed <integer> - randomization seed. use this to get the same exact pattern you did before" << endl;
    std::cout << "  --file <filename.gif> - if an output file is specified, --loop will be enabled" << endl;
    std::cout << "                          and the animation will only loop once, and it won't" << endl;
    std::cout << "                          let you close it until it's done." << endl;
    std::cout << "  --spacepoints <integer> - number of curve anchorpoints in space. " << endl;
    std::cout << "                            increase this to make more complicated shapes.defaults to 30" << endl;
    std::cout << "  --timepoints <integer> - number of curve anchorpoints in time per space anchorpoint." << endl;
    std::cout << "                           increase this to make the loops longer when specifying --loop or --file." << endl;
    std::cout << "                           defaults to 5" << endl;
    std::cout << "  --w <integer> - screen width. defaults to 1000>" << endl;
    std::cout << "  --h <integer> - screen height. defaults to 1000" << endl;
    std::cout << "  --bgcolor <six-digit hex number> - background color - defaults to #ffffff>" << endl;
    std::cout << "  --fgcolor <six-digit hex number> - foreground color - defaults to #0000ff>" << endl;
    std::cout << "  --loop - loops back on itself in time seamlessly" << endl;
    std::cout << "  --incontiguous - spacepoints don't move contiguously through time; they skip pixels" << endl;
    std::cout << "  --pointsperspacecurve <integer> - number of points calculated on each bezier curve." << endl;
    std::cout << "                                    lines are drawn between each point.defaults to 100" << endl;
    std::cout << "  --pointspertimecurve <integer> - number of points calculated on each bezier curve of change." << endl;
    std::cout << "                                   if incontiguous is not specified, points are connected by lines across time." << endl;
    std::cout << "                                   defaults to 100" << endl;
    std::cout << endl;
  }
  GifWriter writer = {};
  if (dowrite)
  ...
  if (noloop)
  {
    ...
    {
      mps[i] = metapoints(w, h, pointsperspacecurve, contiguous);
    }
