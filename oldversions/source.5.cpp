...
class metapoints
{
  ...
  int cpsize;
  point lastpoint;
  metapoints(int w, int h, int pointspercurve)
  {
...
vector<metapoints> metapointspoints[numcurves];

void drawscreen(SDL_Renderer* renderer, int w, int h, vector<point> curvepoints)
{
  for (int x = 0; x < w; x++)
  {
    for (int y = 0; y < h; y++)
